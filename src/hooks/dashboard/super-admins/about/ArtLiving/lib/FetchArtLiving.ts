...
let contentCache: ArtLivingContent[] | null = null;
let lastFetchTime: number | null = null;
const CACHE_DURATION = 5 * 60 * 1000; // 5 menit

export const useArtLivingData = () => {
  const [isLoading, setIsLoading] = useState(true);
  const [contents, setContents] = useState<ArtLivingContent[]>([]);
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const result = await imagekitInstance.upload({
        file: base64,
        fileName: `art-about-content-${Date.now()}`,
        folder: "/art-about-contents",
      });
  ...
  const createContent = async (data: ArtLivingContent, imageUrl: string) => {
    try {
  ...
  const handleUpdate = async (id: string, updatedData: ArtLivingContent) => {
    try {
      const docRef = doc(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_ART_ABOUT as string,
        id
      );
