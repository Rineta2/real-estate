...
let contentCache: AboutTopContent[] | null = null;
let lastFetchTime: number | null = null;
const CACHE_DURATION = 5 * 60 * 1000; // 5 menit

export const useAboutData = () => {
  const [isLoading, setIsLoading] = useState(true);
  const [contents, setContents] = useState<AboutTopContent[]>([]);
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const result = await imagekitInstance.upload({
        file: base64,
        fileName: `about-top-content-${Date.now()}`,
        folder: "/about-top-contents",
      });
  ...
  const createContent = async (data: AboutTopContent, imageUrl: string) => {
    try {
  ...
  const handleUpdate = async (id: string, updatedData: AboutTopContent) => {
    try {
      const docRef = doc(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_TOP_ABOUT as string,
        id
      );
