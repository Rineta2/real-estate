...
let contentCache: CardAboutContent[] | null = null;
let lastFetchTime: number | null = null;
const CACHE_DURATION = 5 * 60 * 1000; // 5 menit

export const useAboutData = () => {
  const [isLoading, setIsLoading] = useState(true);
  const [contents, setContents] = useState<CardAboutContent[]>([]);
  ...
  const fetchContents = useCallback(async (force = false) => {
    ...
    try {
      ...
      const q = query(
        collection(
          db,
          process.env.NEXT_PUBLIC_COLLECTIONS_CARD_ABOUT as string
        ),
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const result = await imagekitInstance.upload({
        file: base64,
        fileName: `about-card-content-${Date.now()}`,
        folder: "/about-card-contents",
      });
  ...
  const createContent = async (data: CardAboutContent, imageUrl: string) => {
    try {
  ...
  const handleUpdate = async (id: string, updatedData: CardAboutContent) => {
    try {
      const docRef = doc(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_CARD_ABOUT as string,
