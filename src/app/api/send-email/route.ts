...
export async function POST(request: NextRequest) {
  try {
    ...
    const { to, subject, message, originalMessage }: EmailPayload =
      await request.json();

    const mailOptions = {
      from: `"Real Estate" <${process.env.GMAIL_USER}>`,
      to: to,
      subject: subject,
      text: `${message}\n\n-------------------\nDetail Pesan:\nNama: ${
        originalMessage.fullName
          ...
          <div style="color: #666;">
            ...
            <table style="margin-top: 10px;">
              ...
              <tr>
                ...
                <td>${new Date(originalMessage.createdAt).toLocaleString(
                  "id-ID"
                )}</td>
              </tr>
            </table>
